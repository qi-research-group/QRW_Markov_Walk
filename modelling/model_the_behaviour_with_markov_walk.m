function [out,varargout] = model_the_behaviour_with_markov_walk(h, r, t, fit_params, SD, plot_it, this_sub, COLLAPSE)
...
sigmasquared = fit_params(4);% .sigmasquared (this is our diffusion)
diffusion = sigmasquared;
% so we keep these fitting parameters the same; the sigmasquared is our
% diffusion; the  Hp_SLOPE is basically our drift rate, POSITIVE (values
% only pos); the Hm_SLOPE is basically our drift rate TO NEG VALUES.
...
S0 = S0_vec(r(2)/10, my_SD);
S0 = S0 / sum(S0); % making sure it sums to 1.0 and nothing else.
...
St = S0;
% how to calculate the ACTUAL rating -- easy; just weighted sum now
R = rating_states * St;
...
for i=1:numel(h) % i.e. the history:
    ...
    % we're using a rather different hamiltonian this time; the markovian
    % one; let's keep the difufsion the same; and the H_SLOPE is now the
    % unfortunate name for the drift; it is either positive, or negative,
    % or 0.
    K = make_K_v2(diffusion, H_SLOPE, numel(S0));
    ...
    % evolve things:
    % evolution also goes a bit differently with the Markov;
    
    U = expm(timestep * time_scaling * K);
    St = U*St;
    
    
    reliability_as_per_Markovmodel = rating_states * St; % (abs(St).^2);
    
    R(end+1) = reliability_as_per_Markovmodel;
    ...
    if rem(i, 28) ==0
        ...
        if COLLAPSE
            ...
            my_tmp = S0_vec(measured_value, my_SD);
            my_tmp = my_tmp / sum(my_tmp);
            
            new_amps_to_use = my_tmp;
            % my_angles = angle(St);
            new_SC = new_amps_to_use;
